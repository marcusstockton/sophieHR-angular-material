...
export class CompaniesClient {
    ...
    getMapFromLatLong(lat: number | undefined, lon: number | undefined): Observable<FileResponse> {
    ...
    postcodeAutoComplete(postcode: string | null | undefined): Observable<FileResponse> {
    ...
    protected processPostcodeAutoComplete(response: HttpResponseBase): Observable<FileResponse> {
        const status = response.status;
        const responseBlob =
            response instanceof HttpResponse ? response.body :
            (response as any).error instanceof Blob ? (response as any).error : undefined;
